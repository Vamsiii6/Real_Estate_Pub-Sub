...
def initializeKafka():
    ...
    topic_list = []
    topic_list.append(
        NewTopic(name="Buffalo", num_partitions=4, replication_factor=3))
    topic_list.append(
        NewTopic(name="Syracuse", num_partitions=4, replication_factor=3))
    topic_list.append(
        NewTopic(name="Albany", num_partitions=4, replication_factor=3))
    topic_list.append(
        NewTopic(name="NYC", num_partitions=1, replication_factor=1))
    admin_client.create_topics(new_topics=topic_list, validate_only=False)
    app.logger.info('Topics created')
    consumer_thread = consumerThread()
    consumer_thread.start()
